import { RecintosZoo } from "../recintos-zoo";

describe("Test: Thiago dos Santos Gomes", () => {
  class MockRecinto {
    getRecinto() {
      return [{ animais: { leao: 2, gazela: 0 } }];
    }
  }

  class MockAnimais {
    getAnimais() {
      return {
        leao: { tamanho: 10 },
        gazela: { tamanho: 3 },
      };
    }
  }

  test("deve retornar informações do animal quando o animal é válido e a quantidade é positiva", () => {
    const zoo = new RecintosZoo();
    zoo.recintos = new MockRecinto();
    zoo.animais = new MockAnimais();

    const resultado = zoo.analisaRecintos("LEAO", 1);

    expect(resultado).toEqual({ tamanho: 10 });
  });
});
